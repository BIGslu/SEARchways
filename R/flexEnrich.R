...
flexEnrich <- function(gene_list = NULL,
  ...
  db_join <- pathway_GOID <- gs_exact_source <- FDR <- gs_name <- n <- db.format <- group <- n_query_genes <- n_background_genes <- gs_cat <- gs_subcat <- pathway <- n_pathway_genes <- n_query_genes_in_pathway <- `k/K` <- pval <- genes <- ensembl_gene <-  gene_symbol <- entrez_gene <- geneID <- NULL
